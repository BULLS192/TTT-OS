...
(function(){
  ...
  var filters={leadSearch:'',leadStatus:'all',leadService:'all',leadSource:'all',oppSearch:'',oppStage:'all',oppService:'all',contactSearch:'',contactType:'all'};
  ...
  function renderOpportunities(){
    ...
    var list=data.opportunities.filter(function(o){
      ...
      return (!q||hay.includes(q))&&(filters.oppStage==='all'||String(o.stage||'').toLowerCase()===filters.oppStage)&&(filters.oppService==='all'||String(o.related_service||o.category||'')===filters.oppService);
    });
    ...
    var rows=list.slice().sort(function(a,b){return new Date(b.updated_at)-new Date(a.updated_at);}).map(function(o){var st=stageInfo(o.stage);return '<tr class="crm-row" data-crm-record="opportunity" data-crm-id="'+attr(o.id)+'"><td><strong>'+html(o.title)+'</strong><br><small>'+html(o.id)+'</small></td><td>'+html(companyName(o.company_id))+'</td><td>'+html(contactName(o.contact_id))+'</td><td>'+html(o.related_service||o.category||'—')+'</td><td><span class="badge">'+html(st.label)+'</span></td><td>'+cash(o.estimated_value)+'</td><td>'+pct(o.probability_pct==null?st.pct:o.probability_pct)+'</td><td>'+cash(Number(o.estimated_value||0)*Number(o.probability_pct==null?st.pct:o.probability_pct)/100)+'</td><td>'+html(o.next_step||'—')+'</td></tr>';}).join('');
    document.getElementById('crmOpportunities').innerHTML='<div class="crm-stage-pills">'+pills+'</div><div class="crm-filters"><input id="crmOppSearch" placeholder="Search opportunities…" value="'+attr(filters.oppSearch)+'"><select id="crmOppStageFilter"><option value="all">All stages</option>'+STAGE_ORDER.map(function(x){return '<option value="'+x+'" '+(filters.oppStage===x?'selected':'')+'>'+html(STAGES[x].label)+'</option>';}).join('')+'</select><select id="crmOppServiceFilter"><option value="all">All services</option>'+services.map(function(x){return '<option '+(filters.oppService===x?'selected':'')+'>'+html(x)+'</option>';}).join('')+'</select></div><div class="panel"><div class="panel-head"><div><h3>Opportunities</h3><p class="muted">Click an opportunity to edit it and review activities, quotes, invoices and linked work.</p></div><span class="badge">'+list.length+' of '+data.opportunities.length+'</span></div><div class="table-wrap"><table><thead><tr><th>Opportunity</th><th>Organization</th><th>Contact</th><th>Service</th><th>Stage</th><th>Value</th><th>Probability</th><th>Weighted</th><th>Next step</th></tr></thead><tbody>'+(rows||'<tr><td colspan="9" class="crm-empty">No opportunities match these filters.</td></tr>')+'</tbody></table></div></div>';
  }
  ...
  function leadDetail(id){
    ...
    return detailHead('LEAD',contactName(l.contact_id),companyName(l.company_id))+'<div class="crm-detail-body">'+
      ...
      '<label>Source detail<input id="crmLeadSourceDetail" value="'+attr(l.source_detail||'')+'"></label><label>Next action<input id="crmLeadNextAction" value="'+attr(l.next_action||'')+'"></label>'+
  ...
  function opportunityDetail(id){
    ...
    return detailHead('OPPORTUNITY',o.title,companyName(o.company_id)+' · '+contactName(o.contact_id))+'<div class="crm-detail-body">'+
      ...
      '<label>Source<input id="crmOppSource" value="'+attr(o.source||'')+'"></label><label>Next step<input id="crmOppNextStep" value="'+attr(o.next_step||'')+'"></label>'+
      '<label>Next step date<input id="crmOppNextDate" type="date" value="'+attr(o.next_step_date||'')+'"></label>'+
  ...
  async function saveOpportunity(id){
    var cloud=window.TTTCloud;if(!cloud?.ready)return;var stage=document.getElementById('crmOpportunityStage')?.value||'prospecting',info=stageInfo(stage);var patch={title:document.getElementById('crmOppTitle')?.value.trim()||('Opportunity '+id),related_service:document.getElementById('crmOppService')?.value.trim()||null,stage:stage,probability_pct:info.pct,estimated_value:Number(document.getElementById('crmOppValue')?.value||0)||null,priority:document.getElementById('crmOppPriority')?.value||'medium',expected_close_date:document.getElementById('crmOppClose')?.value||null,source:document.getElementById('crmOppSource')?.value.trim()||null,next_step:document.getElementById('crmOppNextStep')?.value.trim()||null,next_step_date:document.getElementById('crmOppNextDate')?.value||null,notes:document.getElementById('crmOppNotes')?.value.trim()||null,updated_by:cloud.userId};
