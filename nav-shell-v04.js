(function(){
  ...
  function removeLegacyStandaloneNav(){
    const known=new Set(['expenses','catalog','vendors','scheduling','operations','people','pricing-catalog','pricing','inventory']);
    [...document.querySelectorAll('.os-nav > .nav-item')].forEach(el=>{
      const label=String(el.textContent||'').trim().toLowerCase();
      if(known.has(el.dataset.view)||label==='pricing'||label==='inventory')el.remove();
    });
  }
  ...
  function buildOperationsNav(){
    ...
    items.innerHTML=`
      ...
      <button class="nav-item" type="button" data-shell-view="operations">Operations</button>
  ...
  function buildErpNav(){
    ...
    items.querySelectorAll('[data-erp-tab]').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      window.TTTProductMaster?.openTab?.(btn.dataset.erpTab);
      markShellActive(btn);
      const title={inventory:'Inventory',pricing:'Pricing',review:'Review','purchase-orders':'Purchase Orders'}[btn.dataset.erpTab]||'ERP';
  ...
  function buildFinanceNav(){
    ...
    items.innerHTML=`
      ...
      <button class="nav-item" type="button" data-shell-view="invoices">Invoices</button>
      <button class="nav-item shell-review-nav" type="button" data-review-key="payments">Payments</button>
