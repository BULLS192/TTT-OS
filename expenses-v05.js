...
(function(){
  ...
  function openPolicy(){const admin=window.TTTCloud?.profile?.role==='owner_admin';const m=document.createElement('div');m.className='ex-modal';m.innerHTML=`<div class="ex-dialog"><div class="ex-head"><div><p class="eyebrow">COMPLIANCE CENTER</p><h2>Expense Rules & References</h2></div><button class="btn secondary" id="pc">Close</button></div><div class="ex-grid"><article class="ex-card"><h3>TTT Internal Policy</h3><label>Receipt review threshold ($)<input id="rct" type="number" min="0" step="1" value="${state.policies.receiptThreshold}" ${admin?'':'disabled'}></label><p class="muted">At or above this amount, TTT OS requires either attached receipt/invoice evidence or a documented missing-receipt declaration.</p><label>Alcohol review threshold (% of food + alcohol)<input id="alc" type="number" min="0" max="100" value="${state.policies.alcoholPct}" ${admin?'':'disabled'}></label><p class="muted">${admin?'These thresholds control internal review only. They are not represented as federal or Texas statutory limits.':'Only the TTT OS administrator can change internal expense policy.'}</p>${admin?'<button class="btn primary" id="ps">Save policy</button>':''}</article><article class="ex-card ex-ref"><h3>Official references</h3><a href="${RECORDS}" target="_blank" rel="noopener">IRS — Business Recordkeeping</a><a href="${IRS}" target="_blank" rel="noopener">IRS Publication 463 — Travel, Gift and Car Expenses</a><a href="${TEXAS}" target="_blank" rel="noopener">Texas Comptroller — Sales & Use Tax Records</a><p class="muted">TTT OS keeps the receipt requirement, missing-receipt exception and accounting category separate. Tax treatment remains accountant-reviewed guidance.</p></article></div></div>`;document.body.appendChild(m);m.querySelector('#pc').onclick=()=>m.remove();const ps=m.querySelector('#ps');if(ps)ps.onclick=()=>{state.policies.receiptThreshold=Math.max(0,+m.querySelector('#rct').value||0);state.policies.alcoholPct=Math.max(0,Math.min(100,+m.querySelector('#alc').value||0));save();m.remove();render()}}
