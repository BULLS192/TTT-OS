// TTT Expenses v03 — receipt line items and multi-job allocation
(function(){
  ...
  function enhance(){
    ...
    if(!document.getElementById('exLineStyle')){const s=document.createElement('style');s.id='exLineStyle';s.textContent=
      '.ex-lineitems{background:#fff;border:1px solid #dce4ee;border-radius:13px;padding:14px}.ex-li-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.ex-li-head span{display:block;color:#7a889b;font-size:10px;margin-top:3px}.ex-li-row{display:grid;grid-template-columns:2fr .55fr .8fr 1.1fr 1.55fr 1.2fr 1.5fr 32px;gap:8px;align-items:end;padding:10px 0;border-top:1px solid #edf1f5}.ex-li-row:first-child{border-top:0}.ex-li-row label{min-width:0}.ex-li-remove{height:36px;border:1px solid #e2e8f0;background:#fff;border-radius:8px;cursor:pointer;font-size:18px}.ex-li-summary{display:flex;gap:18px;justify-content:flex-end;border-top:1px solid #e5eaf0;padding-top:10px;margin-top:8px;font-size:11px}.ex-li-summary strong{margin-left:4px}.ex-li-job.hidden{display:none}.ex-li-summary .bad{color:#a35d00}@media(max-width:1050px){.ex-li-row{grid-template-columns:2fr .6fr 1fr 1.3fr}.ex-li-note,.ex-li-job{grid-column:span 2}.ex-li-remove{grid-column:4}}@media(max-width:680px){.ex-li-head{align-items:stretch;flex-direction:column}.ex-li-row{grid-template-columns:1fr 1fr}.ex-li-desc,.ex-li-job,.ex-li-note{grid-column:1/-1}.ex-li-remove{grid-column:2}.ex-li-summary{flex-direction:column;gap:4px}}';document.head.appendChild(s)}
    ...
    function update(){
      [...rows.querySelectorAll('.ex-li-row')].forEach(r=>{const t=r.querySelector('[data-li="allocationType"]').value,j=r.querySelector('.ex-li-job');j.classList.toggle('hidden',t!=='Existing Job')});
      ...
      const badJobs=data.filter(x=>x.allocationType==='Existing Job'&&!x.jobId).length;
      warn.textContent=badJobs?badJobs+' job-allocated item(s) still need a Job selected.':(data.length&&Math.abs(diff)>.01?'Line items do not yet reconcile to the receipt total. Tax/tip can remain at receipt level, so a difference is allowed but should be reviewed.':'');
    }
    function bindRow(r){r.querySelectorAll('input,select').forEach(el=>el.addEventListener('input',update));r.querySelector('.ex-li-remove').onclick=()=>{r.remove();if(!rows.children.length)rows.insertAdjacentHTML('beforeend',rowHTML());[...rows.children].forEach(bindRow);update()}}
