...
const seed={
 ...
 customers:[
  {id:"cus_001",name:"Jordan Lee",phone:"713-555-0184",email:"jordan@example.com",address:"Houston, TX",createdAt:"2026-09-10T14:30:00-05:00"},
  {id:"cus_002",name:"Avery Martin",phone:"832-555-0117",email:"avery@example.com",address:"Houston, TX",createdAt:"2026-09-11T09:15:00-05:00"}
 ],
 vehicles:[
  {id:"veh_001",customerId:"cus_001",year:"2024",make:"Ford",model:"F-150",color:"Black",type:"Truck",vin:"1FTFW1E50RFA00001"},
  {id:"veh_002",customerId:"cus_002",year:"2023",make:"BMW",model:"M4",color:"Blue",type:"Coupe",vin:"WBS43AZ09PCL00002"}
 ],
 workOrders:[
...
document.getElementById("intakeForm").addEventListener("submit",e=>{
 ...
 if(!c){c={id:uid("cus"),name:fd.get("customerName"),phone:fd.get("phone"),email:fd.get("email"),address:fd.get("address"),createdAt:now};db.customers.push(c)}
 let v=db.vehicles.find(x=>x.vin.toLowerCase()===String(fd.get("vin")).toLowerCase());
 if(!v){v={id:uid("veh"),customerId:c.id,vin:fd.get("vin"),year:fd.get("year"),make:fd.get("make"),model:fd.get("model"),color:fd.get("color"),type:fd.get("vehicleType"),keysReceived:fd.get("keysReceived"),odometer:fd.get("odometer"),notes:fd.get("vehicleNotes")};db.vehicles.push(v)}
 const parts=Number(fd.get("parts")||0),labor=Number(fd.get("labor")||0),fees=Number(fd.get("fees")||0);
 const wo={id:"WO-"+new Date().toISOString().slice(2,10).replaceAll("-","")+"-"+String(db.workOrders.length+1).padStart(3,"0"),customerId:c.id,vehicleId:v.id,services:fd.getAll("services"),equipment:fd.get("equipment"),workDetails:fd.get("workDetails"),damageNotes:fd.get("damageNotes"),keysReceived:fd.get("keysReceived"),vehicleNotes:fd.get("vehicleNotes"),estimate:{parts,labor,fees,deposit:Number(fd.get("deposit")||0)},estimateTotal:parts+labor+fees,estimatedCompletion:fd.get("estimatedCompletion"),handoff:fd.get("handoff"),status:"Authorized",assignedTo:"usr_derek",createdBy:"usr_derek",createdAt:now,updatedAt:now,termsVersion:"0.1",termsAccepted:true,signatures:[{role:"customer",name:fd.get("customerSignature"),signedAt:now},{role:"ttt",name:fd.get("staffSignature"),signedAt:now}]};
 ...
 [...document.querySelectorAll('input[type="file"][data-photo]')].forEach(input=>[...input.files].forEach(file=>db.media.push({id:uid("med"),workOrderId:wo.id,category:input.dataset.photo,fileName:file.name,mime:file.type,capturedAt:now,createdBy:"usr_derek",storageStatus:"local-metadata-only"})));
 save();f.reset();document.querySelector('[name="staffSignature"]').value="Derek Thompson";updateTotal();document.querySelectorAll(".photo-preview").forEach(x=>x.innerHTML="");toast(wo.id+" created");show("workorders");
});
